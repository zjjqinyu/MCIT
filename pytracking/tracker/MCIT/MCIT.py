...
class VitTracker(BaseTracker):
    def __init__(self, params):
        super(VitTracker, self).__init__(params)
        network = params.net
    ...
    def initialize(self, img_rgb, img_aux, info: dict):
        ...
        template_aux_crop, _ = sample_target(img_aux, np.array(info['init_bbox']), 
                                         self.params.settings.template_area_factor,
                                         self.params.settings.template_sz)
        self.template_rgb_crop = self._img_process(template_rgb_crop)
        self.template_aux_crop = self._img_process(template_aux_crop)
    ...
    def track(self, img_rgb, img_aux, info: dict = None):
        ...
        with torch.no_grad():
            out_dict = self.network.track(self.template_rgb_crop, self.template_aux_crop, search_rgb_crop, search_aux_crop)

        if self.network.head_type in ['CENTER', 'ACENTER']:
