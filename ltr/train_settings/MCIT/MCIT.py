...
from ltr.models.tracking import MCIT
...
from .MCIT_settings import get_tracker_settings
...
def run(settings):
    ...
    dataset_train = sampler.MultimodalSampler([lasher_train], [1],
                                              samples_per_epoch=20000, max_gap=200, 
    ...
    dataset_val = sampler.MultimodalSampler([lasher_test], [1], 
                                            samples_per_epoch=10000, max_gap=200,
                                            num_search_frames=1, num_template_frames=1,
                                            processing=data_processing_val)
    loader_val = LTRLoader('val', dataset_val, training=False, batch_size=settings.batch_size, num_workers=settings.num_workers,
                            drop_last=True, epoch_interval=10)

    # Create network and actor
    net = MCIT.vit_tracker(settings)
    ...
    param_dicts = [
        ...
        {"params": [p for n, p in net.named_parameters() if "backbone" in n and p.requires_grad], "lr": 1e-5}
    ]
    # param_dicts = net.parameters()
    optimizer = optim.AdamW(param_dicts, lr=1e-4)
    
    lr_scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=40, gamma=0.5)

    trainer = LTRTrainer(actor, [loader_train, loader_val], optimizer, settings, lr_scheduler)
    trainer.train(160, load_latest=True, fail_safe=True)
