...
class MyActor(BaseActor):
    ...
    def compute_loss(self, pred, data):
        if self.net.head_type in ['CENTER', 'ACENTER']:
            # gt gaussian map
...
class CornerActor(BaseActor):
    ...
    def compute_loss(self, pred, data):
        ...
        try:
            ...
            giou_loss, iou = torch.tensor(0.0).to(pred_boxes_vec.device), torch.tensor(0.0).to(pred_boxes_vec.device)
        # compute corner loss
        corner_loss = self.objective['corner'](pred_boxes_vec, gt_boxes_vec)  # (BN,4) (BN,4)
        # weighted sum
        total_loss = self.loss_weight['corner'] * corner_loss
        # status for log
        mean_iou = iou.detach().mean()
        stats = {"Loss/total": total_loss.item(),
                #  "Loss/giou": giou_loss.item(),
